...
func (uq *UserQuery) QueryArticle() *ArticleQuery {
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withArticle: uq.withArticle.Clone(),
...
func (uq *UserQuery) WithArticle(opts ...func(*ArticleQuery)) *UserQuery {
	query := (&ArticleClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withArticle = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withArticle; query != nil {
		if err := uq.loadArticle(ctx, query, nodes,
			func(n *User) { n.Edges.Article = []*Article{} },
			func(n *User, e *Article) { n.Edges.Article = append(n.Edges.Article, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadArticle(ctx context.Context, query *ArticleQuery, nodes []*User, init func(*User), assign func(*User, *Article)) error {
	...
	query.Where(predicate.Article(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.ArticleColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_article
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_article" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_article" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
