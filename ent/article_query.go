...
func (aq *ArticleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Article, error) {
	...
	if aq.withUser != nil || aq.withCategory != nil {
		withFKs = true
	}
	if withFKs {
...
func (aq *ArticleQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Article, init func(*Article), assign func(*Article, *User)) error {
	...
	for i := range nodes {
		if nodes[i].user_article == nil {
			continue
		}
		fk := *nodes[i].user_article
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_article" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (aq *ArticleQuery) loadCategory(ctx context.Context, query *CategoryQuery, nodes []*Article, init func(*Article), assign func(*Article, *Category)) error {
	...
	nodeids := make(map[int64][]*Article)
	for i := range nodes {
		if nodes[i].category_article == nil {
			continue
		}
		fk := *nodes[i].category_article
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "category_article" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
