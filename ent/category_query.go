...
func (cq *CategoryQuery) loadArticle(ctx context.Context, query *ArticleQuery, nodes []*Category, init func(*Category), assign func(*Category, *Article)) error {
	...
	query.Where(predicate.Article(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(category.ArticleColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.category_article
		if fk == nil {
			return fmt.Errorf(`foreign-key "category_article" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "category_article" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
